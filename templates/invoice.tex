...
\outputheader{Invoice}{ {{- invoice_date -}} } % Output the invoice title (automatically all caps) and date (can be empty if not needed)
...
\begin{minipage}[t]{0.38\textwidth}
	\textbf{Due:} \duedatedays{ {{- days_to_pay -}} } % Use the \duedatedays{<number>} command to automatically determine the date when the invoice is due using the number of days from today in the single parameter to the command, or remove it and enter a due date manually
...
\begin{minipage}[t]{0.56\textwidth}
	\textbf{ {{- project.client.name -}} } % Payee name
	
	{{project.client.address.addr1}} \\ % Payee contact lines
{% match project.client.address.addr2 %}
  {% when Some with (addr2) %}
	{{- addr2 -}} \\
  {% when None %}
{% endmatch %}
	{{ project.client.address.city }}, {{ project.client.address.state }} {{ project.client.address.zip }} \\
	\href{mailto: {{ project.client.contact.email -}} }{ {{- project.client.contact.email -}} } % Payee email
\end{minipage}
...
{% for method in me.payment %}
  {% match method %}
	{% when PaymentMethod::Link with {text, url} %}
	\href{ {{- url -}} }{ {{- text -}} }
	{% when PaymentMethod::Text with (text) %}
	{{ text }}
  {% endmatch %}
{% endfor %}
\end{minipage}
